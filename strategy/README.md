...
Each email contains both APEX and NOVA signals in a single HTML message. The subject shows both at a glance:

```
🟡 APEX After-Close Check 2026-05-14 Wed | 53%T 47%V Score+10 NEUTRAL MODERATE  ‖  🟢 NOVA L0 All Clear
🟢 APEX Pre-Market Confirm 2026-05-14 Mon | 100%T 0%V Score+14 NEUTRAL BULLISH  ‖  🟠 NOVA L2 Caution
🚨 APEX After-Close Check 2026-05-14 Fri | 0%T 100%V Score+4 NEUTRAL EMERGENCY EXIT  ‖  🔴 NOVA L3 Bear Alert
```
...
|---|---|---|
| `Weekly Signal` | Sunday ~8 PM ET | Read the full body. Plan Monday's APEX trade and note NOVA level. |
| `Pre-Market Confirm` | Monday 9 AM ET | Check if APEX allocation matches Sunday. Prepare limit order for 10 AM. |
| `Pre-Market Check` | Tue–Fri 9 AM ET | Skim subject. Open body if NOVA level changed or APEX score dropped. |
| `After-Close Check` | 4:15 PM ET daily | Check for `🚨` (APEX emergency) or `🟠/🔴` (NOVA caution/alert). |

**APEX emergency:** If subject starts with `🚨`, open immediately and exit TQQQ → VOO. The email body shows a red banner at the top.

**NOVA alert:** If subject shows `🟠 NOVA L2` or higher, open the NOVA card in the email body and review the signal breakdown and recovery conditions.
...
```
apex_strategy.py          APEX main strategy — run for Sunday signals
sp500_margin_tracker.py   APEX Layer 0 data (FRED CP/GDP) — run quarterly
backtest_ytd.py           APEX YTD backtest engine
backtest_full.py          APEX 15-year full backtest
sp500_output/
  sp500_margin_latest.json    APEX Layer 0 input

nova_strategy.py          NOVA bear monitor — daily semiconductor risk alert
soxx_cycle_tracker.py     NOVA Layer 0 (NVDA+MU quarterly revenue) — auto-run by Action
backtest_nova.py          NOVA bear monitor backtest (5-year episode table)
soxx_output/
  soxx_cycle_latest.json      NOVA Layer 0 input — auto-generated by GitHub Action

notifications/
  email_builder.py            Combined HTML email (APEX + NOVA) — called by GitHub Action
...
# NOVA Bear Monitor — Semiconductor Sector

> **Separate from APEX.** NOVA is a dedicated semiconductor bear risk alert. It does not rotate positions automatically — it detects bear conditions, rates their severity, and gives you the context to decide your own action.
...
A **daily severity alert system** for the semiconductor sector. It does not manage positions or recommend specific trades — it detects bear conditions, rates their severity, and gives you the context to decide your own action.

> **You decide the action. The monitor provides the signal.**

### 5-Level Severity Scale

| Level | Icon | Label | Bear Risk Score | Confirmation | Meaning |
|---|---|---|---|---|---|
| 0 | 🟢 | All Clear | ≤ 1 | — | No defensive action needed |
| 1 | 🟡 | Watch | 2–3 | 3 days | Early softness — monitor closely |
| 2 | 🟠 | Caution | 4–6 | 3 days | Confirmed weakness — consider reducing |
| 3 | 🔴 | Bear Alert | 7–10 | 3 days | Strong bear signal — act if you want protection |
| 4 | ⚫ | Extreme | ≥ 11 | **5 days** | Maximum risk — capital preservation |

Level 4 requires 5-day confirmation (vs 3 for others) because historical data shows it fires prematurely at market bounces within bear markets.

### 7-Signal Bear Risk Score

| Signal | Direction | Why bear-focused |
|---|---|---|
| SOXX 60-day momentum | Downward trend | Sustained decline, filters daily noise |
| EMA50 / EMA200 crossover | Death cross | Major trend reversal (slow, reliable) |
| SOXX vs SPY 60-day | Underperformance | Sector rotation signal — money leaving semis |
| VIX level | Fear | Direct measure of market panic |
| VIX 5-day change | Acceleration | Distinguishes sustained fear from spikes |
| SOXX 6-month drawdown | Capital loss | Measures accumulated damage |
| NVDA 60-day momentum | AI demand proxy | Leading fundamental indicator |

Score range: approximately −7 (maximum bullish) to +15 (maximum bearish).

### Backtest Accuracy (5-Year, 2021–2026)

| Level | Episodes | Avg USD during | Correct calls | False alarms | Signal quality |
|---|---|---|---|---|---|
| 🟡 Watch | 20 | +2.1% | 5 | 5 | Informational only |
| **🟠 Caution** | **18** | **−4.0%** | **9** | **2** | **Best accuracy (82%)** |
| 🔴 Bear Alert | 18 | +4.2% | 5 | 5 | Mixed — acts early |
| ⚫ Extreme | 7 | +5.6% | 1 | 1 | Use with caution |

**Key finding:** Level 2 (Caution) is the most precise actionable signal — 82% hit rate, average USD return of −4.0% during caution episodes. Level 3 fires frequently within bear markets at temporary bounces, making its average USD return misleading (+4.2% includes many short-lived recovery episodes).

**Practical guidance:**
- Level 1 (Watch): no action, just note the deterioration
- Level 2 (Caution): start thinking about reducing exposure — 82% accuracy
- Level 3 (Bear Alert): strong signal but expect some false alarms at bounces
- Level 4 (Extreme): wait for 5-day confirmation before acting

### Notable Backtest Episodes

**Best Level 2 call:** 2022-08-25 → 2022-08-30 — USD fell −18.3% during the episode.

**Worst false alarm:** 2022-10-14 → 2022-10-25 (Level 4, Extreme) — fired right at the market bottom; USD rose +23.3% during the episode. This is why Level 4 now requires 5-day confirmation.

**2022 bear market detection:** The monitor correctly escalated through Watch → Caution → Bear Alert from January through April 2022, well before the worst of the crash.

**Long bull market recognition:** Correctly stayed at Level 0 (All Clear) for:
- 291 consecutive days: 2023-01-06 → 2023-10-24 (USD +110%)
- 279 consecutive days: 2023-11-01 → 2024-08-06 (USD +162%)
- 345 consecutive days: 2025-06-02 → 2026-05-13 (USD +251%)

### Reading the Email

The daily email arrives at 4:30 PM ET (after market close). Subject line format:

```
🟢 NOVA Bear Monitor 2026-05-14 Thu | Level 0 All Clear | Risk score -5 | SOXX $530 (-1% ATH) | BULL
🟠 NOVA Bear Monitor 2022-06-15 Wed | Level 2 Caution | Risk score +5 | SOXX $375 (-22% ATH) | BEAR
🔴 NOVA Bear Monitor 2022-06-15 Wed | Level 3 Bear Alert | Risk score +9 | SOXX $340 (-29% ATH) | BEAR
```

**What to do by level:**

| Email subject shows | What to do |
|---|---|
| 🟢 Level 0 All Clear | Archive. No action needed. |
| 🟡 Level 1 Watch | Skim body. Note which signals are weakening. No trade yet. |
| 🟠 Level 2 Caution | **Read body carefully.** Consider whether to reduce SOXL/USD exposure. |
| 🔴 Level 3 Bear Alert | **Read body immediately.** Review active triggers. Decide defensive action. |
| ⚫ Level 4 Extreme | **Act urgently.** Maximum bear conditions. Wait for 5-day confirmation if Level 4 fires suddenly. |

### Recovery Conditions

When the monitor is at Level 2 or above, the body shows exactly what must clear to de-escalate:

- SOXX must reclaim its 6-month rolling high
- VIX must fall below 22 (Level 3) or 18 (Level 4)
- EMA golden cross must restore (EMA50 > EMA200)
- SOXX 60-day momentum must turn positive
- All conditions must hold for **5 consecutive trading days**

The 5-day recovery requirement means the All Clear is conservative — it won't fire prematurely on a 2-day bounce.

### Usage

```bash
# Daily report (same as GitHub Action)
python nova_strategy.py

# With 5-year backtest episode table
python nova_strategy.py --backtest

# JSON output (for programmatic use)
python nova_strategy.py --json

# YTD performance table (SOXL/USD/SOXX/MU/NVDA)
python nova_strategy.py --ytd

# Update semiconductor cycle data (quarterly, or auto-run by GitHub Action)
python soxx_cycle_tracker.py --save
```

---

## Layer 0 — Semiconductor Cycle Regime

Quarterly regime classification based on NVDA and MU revenue trends (auto-fetched from Yahoo Finance financials):

| Score | Regime | Meaning |
|---|---|---|
| ≥ +4 | 🟢 BULL | Both NVDA and MU revenue expanding strongly |
| −1 to +3 | ⚪ NEUTRAL | Mixed or moderate growth signals |
| ≤ −2 | 🔴 BEAR | One or both revenues contracting |

NVDA revenue is weighted 1.5× (AI/compute demand signal) vs MU revenue 1.0× (memory cycle).

The GitHub Action runs `soxx_cycle_tracker.py --save` before every alert, keeping Layer 0 current after each earnings release.

```bash
python soxx_cycle_tracker.py         # view current cycle
python soxx_cycle_tracker.py --save  # save JSON snapshot for bear monitor
```

---

## GitHub Action — Automated Daily Alerts

One combined job sends a single HTML email with both APEX and NOVA signals (`apex-signal.yml`):

| Schedule | ET time | Purpose |
|---|---|---|
| Sunday 8 PM | `0 0 * * 1` UTC | Weekly plan + regime check |
| Monday 9 AM | `2 13 * * 1` UTC | Pre-market confirm before execution |
| Tue–Fri 9 AM | `2 13 * * 2-5` UTC | Intra-week signal check |
| Mon–Fri 4:15 PM | `15 20 * * 1-5` UTC | After-close trailing stop + bear monitor |

The action auto-runs both `sp500_margin_tracker.py --save` (APEX Layer 0) and `soxx_cycle_tracker.py --save` (NOVA Layer 0) before building the email.
...
*NOVA Bear Monitor v1.0 · Semiconductor sector · Daily severity alert*
